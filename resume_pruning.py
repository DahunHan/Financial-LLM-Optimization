...
# --- 3. LOAD PRUNED MODEL FROM CHECKPOINT ---
# To resume training, we load the model directly from the checkpoint
# instead of creating a new pruned model from scratch.
resume_checkpoint_path = f"{output_dir}/checkpoints/checkpoint-1170"
print(f"Loading pruned model directly from checkpoint: {resume_checkpoint_path}")

# Load the model that has already been pruned and trained for 3 epochs.
base_model = AutoModelForCausalLM.from_pretrained(
    resume_checkpoint_path,
    torch_dtype=torch.bfloat16,
    token=hf_token
)
# The tokenizer is also loaded from the checkpoint directory.
tokenizer = AutoTokenizer.from_pretrained(resume_checkpoint_path, trust_remote_code=True, token=hf_token)
tokenizer.pad_token = tokenizer.eos_token
tokenizer.padding_side = "right"

print(f"\nModel successfully loaded from checkpoint. It has {base_model.config.num_hidden_layers} layers.")
...
training_args = TrainingArguments(
    ...
    num_train_epochs=5, # Fine-tune for a few epochs to let the smaller model adapt.
    learning_rate=2e-5,
...
# The Trainer will automatically continue from the current state of the loaded model.
trainer.train()
